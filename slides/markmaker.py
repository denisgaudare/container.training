...
class Interstitials(object):
    ...
    def __init__(self):
        self.index = 0
        self.images = [url.strip() for url in open("interstitials.txt",encoding="utf8") if url.strip()]
...
def generatefromyaml(manifest, filename):
    ...
    html = open("workshop.template",encoding="utf8").read()
    ...
    online_anchor_path = "https://github.com/denisgaudare/container.training/tree/master"
    ...
    for atatinclude in re.findall(r"@@INCLUDE\[[^]]*\]", html):
        ...
        html = html.replace(atatinclude, open(file_path,encoding="utf8").read())
    return html
...
def processcontent(content, filename):
    if isinstance(content, str):
        ...
        if os.path.isfile(content):
            return processcontent(open(content,encoding="utf8").read(), content)
...
if len(sys.argv) != 2:
    ...
    if filename == "-":
        ...
        manifest = open(filename,encoding="utf8")
